...
BEGIN
    IF current_setting('custom.authorization', true)::boolean THEN
        ...
        -- The per-user RLS helpers viewer_policy(), editor_policy(),
        -- obs_manager_policy(), sensor_policy(), qc_policy(),
        -- add_users_to_policy() and remove_user_from_policy() used to be
        -- defined here. They all operated on policies scoped ``TO
        -- <username>``, which only works when each istSOS user is a
        -- PostgreSQL role. That model is gone: the backend connects as one
        -- service account and runs ``SET LOCAL ROLE <group>`` +
        -- set_config('app.current_user_id', ...). Row-level security is
        -- created once, group-scoped, by
        -- database/migrations/006_session_scoped_rls_policies.sql; custom
        -- per-user rules are managed through POST / DELETE /Policies
        -- (see api/app/v1/endpoints/{create,delete}/policy.py).
        RESET ROLE;
    END IF;
END $BODY$;